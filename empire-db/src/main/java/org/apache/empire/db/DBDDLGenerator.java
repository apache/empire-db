...
public abstract class DBDDLGenerator<T extends DBMSHandler>
{
    ...
    // Options
    protected boolean namePrimaryKeyConstraint = true;  // Add name for primary key constraint
    ...
    protected void createIndex(DBTable t, DBIndex index, DBSQLScript script)
    {
        ...
        // Create Index
        sql.append("CREATE ");
        appendIndexType(index, sql);
        sql.append("INDEX ");
        appendElementName(sql, index.getName());
        sql.append(" ON ");
        t.addSQL(sql, DBExpr.CTX_FULLNAME);
        sql.append(" (");
        ...
        sql.append(")");
        // Create Index
        addCreateIndexStmt(index, sql, script);
    }
